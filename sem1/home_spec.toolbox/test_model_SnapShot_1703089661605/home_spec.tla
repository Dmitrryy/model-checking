...
begin
    ...
    while (TRUE) do
        ...
        or
            petDetected := "yes";
            if (petMotionFeature = "on" /\ systemMode /= "disarmed") then
                alarmState := "sounding";
                userNotified := "notificationSent";
            end if;
        or
...
\* BEGIN TRANSLATION (chksum(pcal) = "8feb4672" /\ chksum(tla) = "f3293894")
...
SensorTrigger == /\ pc["Sensor"] = "SensorTrigger"
                 /\ \/ /\ doorSensor' = "opened"
                    ...
                    \/ /\ petDetected' = "yes"
                       /\ IF (petMotionFeature = "on" /\ systemMode /= "disarmed")
                             THEN /\ alarmState' = "sounding"
                                  /\ userNotified' = "notificationSent"
                             ELSE /\ TRUE
                                  /\ UNCHANGED << alarmState, userNotified >>
...
\* Last modified Wed Dec 20 19:27:24 MSK 2023 by dadro
