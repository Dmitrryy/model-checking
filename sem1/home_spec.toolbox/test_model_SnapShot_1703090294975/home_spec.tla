...
begin
    ...
    while (TRUE) do
        ...
        or
            petDetected := "yes";
            if (petMotionFeature = "on" /\ systemMode = "armedAway") then
                alarmState := "sounding";
                userNotified := "notificationSent";
            end if;
        or
...
\* BEGIN TRANSLATION (chksum(pcal) = "f7e6a295" /\ chksum(tla) = "bc253fcb")
...
SensorTrigger == /\ pc["Sensor"] = "SensorTrigger"
                 /\ \/ /\ doorSensor' = "opened"
                    ...
                    \/ /\ petDetected' = "yes"
                       /\ IF (petMotionFeature = "on" /\ systemMode = "armedAway")
                             THEN /\ alarmState' = "sounding"
                                  /\ userNotified' = "notificationSent"
                             ELSE /\ TRUE
                                  /\ UNCHANGED << alarmState, userNotified >>
...
\* Last modified Wed Dec 20 19:38:08 MSK 2023 by dadro
